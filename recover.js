...
// apps/cli/src/decryptBackupFile.ts
...
var ERR_FACTORS_MISSING = "FACTORS_MISSING";
var ERR_BAD_INPUT = "BAD_INPUT";
var ERR_BAD_NONCE = "BAD_NONCE";
var ERR_BAD_VERSION = "BAD_VERSION";
var ERR_BAD_SUITE = "BAD_SUITE";
var ERR_DECRYPT_FAIL = "DECRYPT_FAIL";
var ERR_ARGON_MISSING_PARAMS = "ARGON_MISSING_PARAMS";
var ERR_ARGON_BAD_TIME = "ARGON_BAD_TIME";
var ERR_ARGON_BAD_MEMORY = "ARGON_BAD_MEMORY";
var ERR_ARGON_BAD_PARALLELISM = "ARGON_BAD_PARALLELISM";
var ERR_ARGON_TIME_TOO_HIGH = "ARGON_TIME_TOO_HIGH";
var ERR_ARGON_MEMORY_TOO_HIGH = "ARGON_MEMORY_TOO_HIGH";
var ERR_ENVELOPE_MISSING = "ENVELOPE_MISSING";
var ERR_STORAGE_FORMAT = "STORAGE_FORMAT";
...
var b64urlToU8 = (s) => {
  ...
  else if (pad !== 0) throw new CryptoError(ERR_BAD_INPUT, "Invalid base64url string length");
  return b64ToU8(b64);
};
var hexToU8 = (hex) => {
  let s = hex.trim();
  ...
  if (s.length % 2 !== 0) throw new CryptoError(ERR_BAD_INPUT, "hex length must be even");
...
var unwrapBytea = (v) => {
  ...
  if (v instanceof Uint8Array) {
    const clean = new Uint8Array(v.length);
    clean.set(v);
    return clean;
  }
  let s = v.trim();
  if (s.startsWith('"') && s.endsWith('"')) s = s.slice(1, -1);
  if (s.startsWith("\\x") || s.startsWith("0x")) return hexToU8(s);
  const tryB64 = (txt) => {
    try {
      const arr = b64ToU8(txt);
      const clean = new Uint8Array(arr.length);
      clean.set(arr);
      return clean;
    } catch {
      return null;
  ...
  const direct = tryB64(s);
  if (direct) return direct;
  const pad = s.length % 4 ? "=".repeat(4 - s.length % 4) : "";
  const fromUrl = tryB64(s.replace(/-/g, "+").replace(/_/g, "/") + pad);
  if (fromUrl) return fromUrl;
  throw new CryptoError(ERR_BAD_INPUT, `unwrapBytea: unrecognized binary format`);
};

// packages/crypto/src/zeroize.ts
function zeroMany(...items) {
...
function expectBytes(field, label) {
  const value = unwrapBytea(field);
  if (!(value instanceof Uint8Array) || value.length === 0) {
    throw new CryptoError(
      ERR_STORAGE_FORMAT,
      `${label} missing or invalid in vault row.`
    );
  }
  return value;
}
function normalizeVault(row) {
  return {
    ...
    email: row.email,
    secretType: row.secret_type ?? null,
    payloadCiphertext: expectBytes(row.payload_ciphertext, "payload_ciphertext"),
    payloadNonce: expectBytes(row.payload_nonce, "payload_nonce"),
    payloadVersion: row.payload_version,
    pkEnvelopeCiphertext: expectBytes(row.pk_envelope_ciphertext, "pk_envelope_ciphertext"),
    pkEnvelopeNonce: expectBytes(row.pk_envelope_nonce, "pk_envelope_nonce"),
    pkEnvelopeAAD: expectBytes(row.pk_envelope_aad, "pk_envelope_aad"),
    pkEnvelopeVersion: row.pk_envelope_version,
    pwdpkEnvelopeCiphertext: expectBytes(
      row.pwdpk_envelope_ciphertext,
      "pwdpk_envelope_ciphertext"
    ),
    pwdpkEnvelopeNonce: expectBytes(
      row.pwdpk_envelope_nonce,
      "pwdpk_envelope_nonce"
    ),
    pwdpkEnvelopeAAD: expectBytes(
      row.pwdpk_envelope_aad,
      "pwdpk_envelope_aad"
    ),
    pwdpkEnvelopeVersion: row.pwdpk_envelope_version,
    metaEnvelopeCiphertext: expectBytes(
      row.meta_envelope_ciphertext,
      "meta_envelope_ciphertext"
    ),
    metaEnvelopeNonce: expectBytes(
      row.meta_envelope_nonce,
      "meta_envelope_nonce"
    ),
    metaEnvelopeAAD: expectBytes(
      row.meta_envelope_aad,
      "meta_envelope_aad"
    ),
    metaEnvelopeVersion: row.meta_envelope_version,
    argonSalt: expectBytes(row.argon_salt, "argon_salt"),
    argonTime: row.argon_time,
    argonMemMiB: row.argon_mem_mib,
    argonParallelism: row.argon_parallelism,
    argonVersion: row.argon_version,
    suite: row.suite ?? 1,
    aadVersion: row.aad_version ?? 1,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
    lastDecryptedAt: row.last_decrypted_at,
    kdfSalt: expectBytes(row.kdf_salt, "kdf_salt")
  };
}
...
// packages/sdk/src/buildDecryptInput.ts
function buildDecryptInput(vault, factors) {
  if (!factors.passkeyPrf && !factors.pwdpkKey) {
    throw new CryptoError(
      ERR_FACTORS_MISSING,
      "No factors provided to build decrypt input."
    );
  }
  const payload = {
    ciphertext: vault.payloadCiphertext,
    nonce: vault.payloadNonce,
    version: vault.payloadVersion
  };
  const pkCandidate = vault.pkEnvelopeCiphertext ? {
    version: vault.pkEnvelopeVersion,
    ciphertext: vault.pkEnvelopeCiphertext,
    nonce: vault.pkEnvelopeNonce,
    aad: vault.pkEnvelopeAAD
  } : void 0;
  const pwdpkCandidate = vault.pwdpkEnvelopeCiphertext ? {
    version: vault.pwdpkEnvelopeVersion,
    ciphertext: vault.pwdpkEnvelopeCiphertext,
    nonce: vault.pwdpkEnvelopeNonce,
    aad: vault.pwdpkEnvelopeAAD
  } : void 0;
  const metaCandidate = vault.metaEnvelopeCiphertext && vault.metaEnvelopeNonce && vault.metaEnvelopeAAD ? {
    version: vault.metaEnvelopeVersion,
    ciphertext: vault.metaEnvelopeCiphertext,
    nonce: vault.metaEnvelopeNonce,
    aad: vault.metaEnvelopeAAD
  } : void 0;
  if (!pkCandidate) {
    throw new CryptoError(
      ERR_ENVELOPE_MISSING,
      "Missing pk envelope in vault storage."
    );
  }
  if (!pwdpkCandidate) {
    throw new CryptoError(
      ERR_ENVELOPE_MISSING,
      "Missing pwdpk envelope in vault storage."
    );
  }
  if (!metaCandidate) {
    throw new CryptoError(
      ERR_ENVELOPE_MISSING,
      "Missing meta envelope in vault storage."
    );
  }
  const pkEnvelope = pkCandidate;
  const pwdpkEnvelope = pwdpkCandidate;
  const metaEnvelope = metaCandidate;
  const expected = {
    payloadVersion: VERSION.PAYLOAD,
    pkEnvelopeVersion: VERSION.ENVELOPE,
    pwdpkEnvelopeVersion: VERSION.ENVELOPE,
    metaEnvelopeVersion: VERSION.ENVELOPE
  };
  const kdfSalt = vault.kdfSalt;
  if (!(kdfSalt instanceof Uint8Array) || kdfSalt.length === 0) {
    throw new CryptoError(
      ERR_STORAGE_FORMAT,
      "Invalid kdfSalt in vault storage."
    );
  }
  return {
    payload,
    ...
    suite: vault.suite,
    kdfSalt,
    passkeyPrf: factors.passkeyPrf,
    pwdpkKey: factors.pwdpkKey,
    expected
  };
}

// packages/crypto/src/envelope.ts
function validateEnvelope(env, expectedVersion) {
  if (typeof env.version !== "number" || !Number.isFinite(env.version)) {
    throw new CryptoError(
      ERR_BAD_VERSION,
      `Invalid envelope version value: ${String(env.version)}`
    );
  }
  if (env.nonce.length !== 12) {
    throw new CryptoError(
      ERR_BAD_NONCE,
      `Invalid nonce length: ${env.nonce.length}`
    );
  }
  if (env.ciphertext.length < 16) {
    throw new CryptoError(
      ERR_DECRYPT_FAIL,
      `Ciphertext too short: ${env.ciphertext.length}`
    );
  }
  if (env.version !== expectedVersion) {
    throw new CryptoError(
      ERR_BAD_VERSION,
      `Envelope version mismatch: stored=${env.version}, expected=${expectedVersion}`
    );
  }
}
...
async function hkdfAesKey({
  ...
  info,
  extractable = false
  // default non-extractable
}) {
  if (!(ikm instanceof Uint8Array)) throw new CryptoError(ERR_BAD_INPUT, "hkdfAesKey: ikm must be Uint8Array");
  if (!(salt instanceof Uint8Array)) throw new CryptoError(ERR_BAD_INPUT, "hkdfAesKey: salt must be Uint8Array");
...
var toArrayBuffer = (u8) => {
  const ab = new ArrayBuffer(u8.byteLength);
  new Uint8Array(ab).set(u8);
  return ab;
};
var decryptGcm = async ({ key, ct, nonce, aad }) => {
  if (!(nonce instanceof Uint8Array) || nonce.byteLength !== NONCE_BYTES) {
    throw new CryptoError(
      ERR_BAD_NONCE,
      `Invalid nonce length: ${nonce?.byteLength ?? "null"}`
    );
  }
  const algo = {
    name: "AES-GCM",
  ...
  try {
    ...
    if (e instanceof Error && e.name === "OperationError") {
      throw new CryptoError(ERR_DECRYPT_FAIL, "AES-GCM decryption failed.");
    }
    throw e;
  }
};

// packages/flows/src/decryptVault.ts
async function decryptVault(input) {
  ...
  const INFO = HKDF_INFO_BY_SUITE[suite];
  if (!INFO) {
    throw new CryptoError(
      ERR_BAD_SUITE,
      `Unknown suite version: ${suite}. This vault was encrypted with a newer version of Keypsafe.`
    );
  }
  if (!passkeyPrf && !pwdpkKey) {
    throw new CryptoError(
      ERR_FACTORS_MISSING,
      "No factors provided to decrypt vault."
    );
  }
  if (!(kdfSalt instanceof Uint8Array) || kdfSalt.length === 0) {
    throw new CryptoError(
      ERR_STORAGE_FORMAT,
      "Invalid kdfSalt provided for vault decryption."
    );
  }
  if (payload.version !== expected.payloadVersion) {
    throw new CryptoError(
      ERR_BAD_VERSION,
      `Unexpected payload version: got ${payload.version}, expected ${expected.payloadVersion}.`
    );
  }
  let DEK = null;
  let metaPlaintext = null;
  try {
    if (expected.pkEnvelopeVersion !== void 0) {
      validateEnvelope(pkEnvelope, expected.pkEnvelopeVersion);
    }
    if (expected.pwdpkEnvelopeVersion !== void 0) {
      validateEnvelope(pwdpkEnvelope, expected.pwdpkEnvelopeVersion);
    }
    validateEnvelope(metaEnvelope, expected.metaEnvelopeVersion);
    if (passkeyPrf) {
      const wrapKeyPK = await hkdfAesKey({
        ikm: passkeyPrf,
        salt: kdfSalt,
        info: INFO.KEK_PK
      });
      try {
        DEK = await decryptGcm({
          key: wrapKeyPK,
          ct: pkEnvelope.ciphertext,
          nonce: pkEnvelope.nonce,
          aad: pkEnvelope.aad
        });
      } catch {
        DEK = null;
    ...
    if (!DEK && pwdpkKey) {
      const wrapKeyPWDPK = await hkdfAesKey({
        ikm: pwdpkKey,
        salt: kdfSalt,
        info: INFO.KEK_PWDPK
      });
      try {
        DEK = await decryptGcm({
          key: wrapKeyPWDPK,
          ct: pwdpkEnvelope.ciphertext,
          nonce: pwdpkEnvelope.nonce,
          aad: pwdpkEnvelope.aad
        });
      } catch {
        DEK = null;
      }
    ...
    if (!DEK) {
      throw new CryptoError(
        ERR_DECRYPT_FAIL,
        "Failed to unwrap DEK with provided factors."
      );
    }
    const metaKey = await hkdfAesKey({
      ikm: DEK,
      salt: kdfSalt,
      info: INFO.META
    });
    metaPlaintext = await decryptGcm({
      key: metaKey,
      ct: metaEnvelope.ciphertext,
      nonce: metaEnvelope.nonce,
      aad: metaEnvelope.aad
    });
    let metaObj;
    try {
      metaObj = JSON.parse(td.decode(metaPlaintext));
    } catch {
      throw new CryptoError(
        ERR_DECRYPT_FAIL,
        "Failed to parse meta envelope."
      );
    }
    if (metaObj.kdf_salt_b64url) {
      let metaKdfSalt;
      try {
        metaKdfSalt = b64urlToU8(metaObj.kdf_salt_b64url);
      } catch {
        throw new CryptoError(
          ERR_STORAGE_FORMAT,
          "Invalid kdf_salt_b64url in meta."
        );
      }
      let diff = metaKdfSalt.length !== kdfSalt.length ? 1 : 0;
      for (let i = 0; i < metaKdfSalt.length; i++) {
        diff |= metaKdfSalt[i] ^ kdfSalt[i];
      }
      if (diff !== 0) {
        throw new CryptoError(
          ERR_DECRYPT_FAIL,
          "kdfSalt mismatch between storage and metadata (possible tampering)."
        );
      }
    }
    const payloadKey = await hkdfAesKey({
      ikm: DEK,
      // payload bound to META AAD
      salt: kdfSalt,
      info: INFO.DEK_PAYLOAD
    });
    ...
    if (DEK) zeroMany(DEK);
    if (metaPlaintext) zeroMany(metaPlaintext);
  }
}

// packages/crypto/src/argon.ts
var import_hash_wasm = __toESM(require_index_umd(), 1);
var ARGON_MAX_TIME = 20;
var ARGON_MAX_MEM_MIB = 256;
function validateArgonParams(params) {
  if (!params) {
    throw new CryptoError(
      ERR_ARGON_MISSING_PARAMS,
      "Argon2 params are missing."
    );
  }
  const { time, memMiB, parallelism } = params;
  if (typeof time !== "number" || !Number.isFinite(time) || time < 1) {
    throw new CryptoError(
      ERR_ARGON_BAD_TIME,
      "Argon2 time must be >= 1."
    );
  }
  if (typeof memMiB !== "number" || !Number.isFinite(memMiB) || memMiB < 16) {
    throw new CryptoError(
      ERR_ARGON_BAD_MEMORY,
      "Argon2 memMiB must be >= 16."
    );
  }
  if (time > ARGON_MAX_TIME) {
    throw new CryptoError(
      ERR_ARGON_TIME_TOO_HIGH,
      `Argon2 time must be <= ${ARGON_MAX_TIME}.`
    );
  }
  if (memMiB > ARGON_MAX_MEM_MIB) {
    throw new CryptoError(
      ERR_ARGON_MEMORY_TOO_HIGH,
      `Argon2 memMiB must be <= ${ARGON_MAX_MEM_MIB}.`
    );
  }
  if (typeof parallelism !== "number" || parallelism !== 1) {
    throw new CryptoError(
      ERR_ARGON_BAD_PARALLELISM,
      "Argon2 parallelism must be exactly 1 for hash-wasm."
    );
  }
  return params;
}
async function deriveKekPwd({
  password,
  salt,
  params
}) {
  const safe = validateArgonParams(params);
  const memoryKiB = safe.memMiB * 1024;
  const effectiveParallelism = 1;
  const out = await (0, import_hash_wasm.argon2id)({
    password,
    // UTF-8 handled internally
    salt,
    // Uint8Array
    parallelism: effectiveParallelism,
    // always 1
    iterations: safe.time,
    // e.g., 3
...
// packages/sdk/src/derivePwdpkKey.ts
async function derivePwdpkKey({
  password,
  paperKeyB64url,
  argonSalt,
  argonTime,
  ...
  if (!pw || !paperKeyB64url.trim()) {
    throw new CryptoError(
      ERR_FACTORS_MISSING,
      "Password and paper key are required for pwd+paper derivation."
    );
  }
  if (!(argonSalt instanceof Uint8Array) || argonSalt.length === 0) {
    throw new CryptoError(
      ERR_STORAGE_FORMAT,
      "Invalid Argon salt for pwd+paper derivation."
    );
  }
  const params = {
    time: argonTime,
  ...
  let paperBytes;
  try {
    paperBytes = b64urlToU8(paperKeyB64url.trim());
  } catch {
    throw new CryptoError(
      ERR_STORAGE_FORMAT,
      "Invalid paperKeyB64url encoding."
    );
  }
  const kPwd = await deriveKekPwd({
    password: pw,
    salt: argonSalt,
    params
  });
  const combo = new Uint8Array(kPwd.length + paperBytes.length);
  combo.set(kPwd, 0);
  combo.set(paperBytes, kPwd.length);
  zeroMany(kPwd, paperBytes);
  return combo;
}

// apps/cli/src/decryptBackup.ts
async function decryptBackupVault(backup, opts) {
  const { password, paperKeyB64url } = opts;
  if (!password) {
    throw new Error("Password is required.");
  }
  if (!paperKeyB64url) {
    throw new Error("Paper key (base64url) is required.");
  }
  const storedVault = normalizeVault(backup.vault);
  const pwdpkKey = await derivePwdpkKey({
    password,
    paperKeyB64url: paperKeyB64url.trim(),
    argonSalt: storedVault.argonSalt,
    argonTime: storedVault.argonTime,
    argonMemMiB: storedVault.argonMemMiB,
    argonParallelism: storedVault.argonParallelism
  });
  try {
    const input = buildDecryptInput(storedVault, {
      pwdpkKey
    });
    const { plaintext } = await decryptVault(input);
    const text = td.decode(plaintext);
    zeroMany(plaintext);
    return text;
  } finally {
    zeroMany(pwdpkKey);
...
function parseArgs(args) {
  ...
  if (outputIdx !== -1) {
    outputFile = rest[outputIdx + 1];
    rest.splice(outputIdx, 2);
  }
  const stdoutIdx = rest.indexOf("--stdout");
  if (stdoutIdx !== -1) {
    useStdout = true;
    rest.splice(stdoutIdx, 1);
  }
  const backupPath = rest[0];
  if (!backupPath) {
    throw new Error(
      "Usage: node recover.js <backup.json> (--output <file> | --stdout)"
    );
  }
  if (!outputFile && !useStdout) {
    throw new Error(
      "Specify where to send the output:\n  --output <file>   recommended \u2014 writes to a file, nothing appears on screen\n  --stdout          prints to terminal (clears screen after you press Enter)"
    );
  }
...
async function runCli(opts) {
  ...
  const plaintext = await decryptBackupVault(parsed, { password, paperKeyB64url });
  if (outputFile) {
    const outAbs = path.resolve(outputFile);
    fs.writeFileSync(outAbs, plaintext, { encoding: "utf8", mode: 384 });
    process.stderr.write(`Decrypted secret written to: ${outAbs}
`);
    process.stderr.write("Delete or move the file to a secure location when you are done.\n");
  } else if (useStdout) {
    writeStdout(plaintext + "\n");
    if (process.stdout.isTTY) {
      await new Promise((resolve2) => {
        ...
        rl.question(
          "\nCopy your secret above, then press Enter to clear the terminal.",
          () => {
            rl.close();
            resolve2();
          }
...
if (fileURLToPath(import.meta.url) === path.resolve(process.argv[1])) {
  main().catch((err) => {
    console.error("Decrypt failed:", err?.message || err);
    process.exit(1);
  });
}
export {
